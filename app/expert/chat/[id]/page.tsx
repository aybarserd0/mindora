...
export default function ExpertChatPage({
  ...
  function getAttachmentSignedUrlCandidates(id: string, attachmentId: string) {
    ...
    return [
      `/api/conversations/${id}/attachments/${attachmentId}/signed?role=expert&token=${encodedToken}`,
      `/api/conversations/${id}/attachments/${attachmentId}/signed-url?role=expert&token=${encodedToken}`,
      `/api/conversations/${id}/attachments/${attachmentId}?role=expert&token=${encodedToken}`,
    ]
  }
  ...
  async function getSignedAttachmentUrl(attachment: Attachment) {
    ...
    for (const url of getAttachmentSignedUrlCandidates(conversationId, attachment.id)) {
      try {
        ...
        console.error('EXPERT ATTACHMENT SIGNED URL CANDIDATE ERROR:', {
  ...
  async function handleStartVideoSession() {
    ...
    try {
      ...
      router.push(`/expert/session/${data.session.id}?token=${getEncodedAccessToken()}`)
    } catch (err) {
      console.error('EXPERT VIDEO SESSION START ERROR:', err)
