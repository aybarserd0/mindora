...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return jsonError('Bildirimler okundu olarak işaretlenemedi.', 500)
    }

    return NextResponse.json({
      ok: true,
      updatedCount: Array.isArray(data) ? data.length : 0,
      updatedIds: Array.isArray(data) ? data.map((item: { id: string }) => item.id) : [],
    })
  } catch (error) {
    console.error('NOTIFICATIONS_READ_ALL_UNEXPECTED_ERROR', error)
